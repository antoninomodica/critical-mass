...
setoutletassist(3,"series of x/y and index lists");
...
function bang()
{
	...
	//converts all coordinates to polar and stores them in the spherical Array
	for (i=0;i<myagentcount;i++) {
		spherical[i] = cartesian2Polar(agents[i].x, agents[i].y);
	}

//outputs current positions and velocities
	for (i=0;i<myagentcount;i++) {
		outlet(0,agents[i].x,agents[i].y,agents[i].vx,agents[i].vy);
	}

	for (i=0;i<myagentcount;i++) {
		outlet(3, i, agents[i].x,agents[i].y);
	}
}
	...
	for (i=0;i<myagentcount;i++)
	{
		if (a != agents[i])
		{
			...
			if ((Math.abs(dx)>0.0001)&&(Math.abs(dy)>0.0001))
				mag = Math.sqrt((dx*dx+dy*dy)); // cheap mag, no sqrt
			else
				mag = 0.01;
